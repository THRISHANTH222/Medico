...
export async function POST() {
  try {
    ...
    if (!livekitUrl) {
      console.error('[LiveKit Token API] Missing LIVEKIT_URL or NEXT_PUBLIC_LIVEKIT_URL in environment variables.');
      return NextResponse.json(
    ...
    const roomName = `rural-health-${randomId}`;
    const participantIdentity = `user-${randomId}`;

    const at = new AccessToken(apiKey, apiSecret, {
      identity: participantIdentity,
      name: 'Rural Health User',
      ttl: '15m',
    });
    ...
    console.log(`[LiveKit Token API] Token generated for room: ${roomName}`);
